...
def main():
    ...
    for k in range(len(t)):
        ...
        # if k % 50 == 0:
        #     theta += np.random.normal(0, 1e-6, size=theta.shape)  # simulate parameter drift
        #     quad.set_estimated_inertial_params(theta)  # update quadrotor with new parameters
        ...
        A_mat = A_mat / A_norm
        b_vec = quad.get_force_vector(x_meas, u) #+ np.random.normal(0, 1e-17, size=(A_mat.shape[0],))
        b_vec = b_vec / A_norm
        ...
        # perturb theta
        if k == event_step:
            # quad.aero_added_inertia(1) # wind speed

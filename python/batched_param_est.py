...
BASE_SEED = 12345
N_TRIALS  = 100
ALGOS     = ["none", "lstsq", "rk", "tark", "rls", "kf", "gt"]  # includes "none" and "gt" 
EVENT = "wind"  # "payload" or "wind" or "drift"
...
def run_single_trial(estimator_option: str, trial_idx: int):
    ...
    Returns per-time arrays:
      ...
      gt_force_err_t, est_force_err_t (NaN if none/gt before estimate)
    """
    ...
    if estimator_option == "rls":
        ...
        estimator = KF(num_params=theta_true.shape[0], process_noise=1e-4*np.eye(theta_true.shape[0]),
                       measurement_noise=1e-4*np.eye(6), theta_hat=theta_true, c=1000)
    ...
    # "lstsq" handled inline; "gt" & "none" skip estimator
    ...
    for k in range(len(t)):
        ...
        # estimator update (every 20 steps)
        if k % 20 == 0:
            if estimator_option == "gt":
            ...
            elif estimator_option in ("rk", "tark", "rls", "kf"):
                ...
                theta_est = None  # explicitly keep None; no updates to ug/LQR

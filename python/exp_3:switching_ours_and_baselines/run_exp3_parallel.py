...
ALGOS_DEFAULT: List[str] = ["rls_0.99", "rls_0.96", "kf_low", "kf_high", "tagrk"] # "rk", "grk", "tark",  
...
def get_theta_vector(est):
    """Return current parameter estimate as a 1-D np.ndarray, or None."""
    if hasattr(est, "theta_hat") and est.theta_hat is not None:
        return np.asarray(est.theta_hat, dtype=float).reshape(-1)
    if hasattr(est, "x0") and est.x0 is not None:
        return np.asarray(est.x0, dtype=float).reshape(-1)
    return None

def set_theta_vector(est, theta_vec: np.ndarray):
    """Overwrite the estimator's parameter vector in-place."""
    theta_vec = np.asarray(theta_vec, dtype=float).reshape(-1)
    if hasattr(est, "theta_hat"):
        est.theta_hat = theta_vec.copy()
    elif hasattr(est, "x0"):
        est.x0 = theta_vec.copy()
    else:
        raise AttributeError("Estimator has neither 'theta_hat' nor 'x0'")
...
def parse_substitution_name(estimator_name: str) -> tuple[str, str | None]:
    ...
    Returns (mode, partner) where:
      ...
      mode == "none"                and partner == None

    Naming conventions:
      - "<baseline>_sub_by_tagrk"
      - "tagrk_sub_by_kf_high"
      - "tagrk_sub_by_rls_0.96"
    """
...
def base_algo_name(estimator_name: str) -> str:
    """Strip our ablation suffixes and return the primary algo to instantiate."""
    name = estimator_name.lower()
    if name.endswith("_sub_by_tagrk") and not name.startswith("tagrk"):
...
def run_single_trial(
    ...
    *,
    ...
    # Parse substitution mode and instantiate the primary estimator accordingly
    mode, partner = parse_substitution_name(estimator_name)
    primary_name = base_algo_name(estimator_name)

    theta0 = quad.get_true_inertial_params()
    est = make_estimator(primary_name, theta0, window)   # primary

    # Substitution state
    sub_active = False
    sub_left = 0
    sub_est = None        # will hold partner estimator during sub windows
    event_pending = False # becomes True at event steps; consumed at next estimate
        ...
        for k in range(len(t)):
            ...
            if (k % est_freq == 0) and (k > 0):
                ...
                if estimator_name == "gt":
                    ...
                    # If an event happened since last update, (re)start a K-step substitution
                    if event_pending:
                        # Close any ongoing substitution cleanly: push sub-est back to primary
                        if sub_active and sub_est is not None:
                            th_sub = get_theta_vector(sub_est)
                            if th_sub is not None:
                                set_theta_vector(est, th_sub)

                        # Initialize a NEW substitution depending on 'mode'
                        if mode == "baseline_by_tagrk":
                            # Primary is a baseline; partner is TAGRK
                            th0 = get_theta_vector(est)
                            sub_est = make_estimator("tagrk", th0 if th0 is not None else theta0, window)
                            sub_active = True
                            sub_left = max(1, sub_k)

                        elif mode == "tagrk_by_partner" and partner is not None:
                            # Primary is TAGRK; partner is KF or RLS
                            th0 = get_theta_vector(est)  # TAGRK uses x0
                            sub_est = make_estimator(partner, th0 if th0 is not None else theta0, window)
                            sub_active = True
                            sub_left = max(1, sub_k)
                    ...
                    # --------- run one estimation update ---------
                    used_estimator = est
                    if sub_active and (sub_est is not None):
                        # update with the substitute for this window
                        theta_est = sub_est.iterate(A_stack, b_stack)
                        used_estimator = sub_est

                        # one step done in substitute mode
                        sub_left -= 1
                        if sub_left <= 0:
                            # push final sub estimate back into the primary and end substitution
                            th_sub = get_theta_vector(sub_est)
                            if th_sub is not None:
                                set_theta_vector(est, th_sub)
                            sub_active = False
                        ...
                        # normal primary update
                        theta_est = est.iterate(A_stack, b_stack)

                    # post-process the *currently-used* estimate for controller gating
                    theta_est_post = theta_est.copy()
                    theta_est_post[4] = theta_est_post[5] = np.mean(theta_est_post[4:6])
                    theta_est_post = closest_spd(theta_est_post)

                    if gate_param_update(theta_est_post, A_stack, b_stack):
                        # Apply to physical model + LQR
                        quad.set_estimated_inertial_params(theta_est_post)
                        ug = quad.get_hover_thrust_est()
                        A_new, B_new = quad.get_linearized_est(xg, ug)
                        lqr.update_linearized_dynamics(A_new, B_new)

                    # Log residual using the parameter actually used this step
                    est_err_traj.append(rel_residual(A_stack, theta_est_post, b_stack))
                    theta_est_cur = theta_est_post.copy()
...
def main():
    ...
    ap.add_argument("--sub_k", type=int, default=2,
                help="Number of consecutive estimation updates to substitute after each event.")
        ...
        for a_idx, algo in enumerate(ALGOS):
            ...
            if workers > 0:
                ...
                with ProcessPoolExecutor(max_workers=workers) as ex, tqdm(total=N_TRIAL, desc=desc) as pbar:
                    futures = [
                        ex.submit(_trial_worker, algo, BASE_SEED, seed_offset, est_freq, window, ref_type, T_sim, noise, sub_k)

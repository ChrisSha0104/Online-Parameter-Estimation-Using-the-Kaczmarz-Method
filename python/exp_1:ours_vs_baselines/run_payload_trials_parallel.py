...
ALGOS_DEFAULT: List[str] = [ "rls_0.5", "rls_0.3", "kf_low", "kf_high", "tagrk"] # "rk", "grk", "tark",  
...
def make_estimator(name: str, theta0: np.ndarray, window: int):
    ...
    if name == "rls_0.5":
        return RLS(num_params=n, theta_hat=theta0, forgetting_factor=0.5, c=10)
    if name == "rls_0.3":
        return RLS(num_params=n, theta_hat=theta0, forgetting_factor=0.3, c=10)
    if name == "kf_low":
        Q = 1e-3 * np.eye(n)
